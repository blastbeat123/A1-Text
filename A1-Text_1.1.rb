...
class Wordprocessor
  ...
  # Setup Groq
  groq_key = CONFIG.dig('groq_api', 'api_key')
  if groq_key && !groq_key.empty? && groq_key != "b"
    @ai_providers[:groq] = {
      api_key: groq_key,
      url: CONFIG.dig('groq_api', 'url') || 'https://api.groq.com/openai/v1/chat/completions',
      model: CONFIG.dig('groq_api', 'model') || 'llama-3.3-70b-versatile',
      name: "Groq (#{CONFIG.dig('groq_api', 'model')})",
      type: :groq
    }
    puts "  ✅ Groq configurato: #{@ai_providers[:groq][:model]}"
  else
    puts "  ⚠️  Groq: chiave API non trovata"
  end
  ...
  if gemini_key && !gemini_key.empty? && gemini_key != "INSERISCI_QUI_LA_TUA_CHIAVE_API"
    @ai_providers[:gemini] = {
      ...
      model: CONFIG.dig('google_api', 'model') || 'gemini-3-flash-preview:generateContent',
      name: "Gemini (#{CONFIG.dig('google_api', 'model')})",
      type: :gemini
    }
    puts "  ✅ Gemini configurato: #{@ai_providers[:gemini][:model]}"
  else
    puts "  ⚠️  Gemini: chiave API non trovata"
  end

  @ai_configured = !@ai_providers.empty?

  if @ai_configured
    puts "✅ Provider AI disponibili: #{@ai_providers.keys.join(', ')}"
    @current_ai_provider = @ai_providers.keys.first
    puts "   Provider predefinito: #{@current_ai_provider}"
  else
    puts "❌ Nessun provider AI configurato. Funzionalità AI disabilitate."
  end
end
...
@text.buffer.signal_connect('insert-text') do |buffer, iter, text, len|
  ...
  def setup_context_menu
     ...
     return unless @ai_providers[:gemini]  # Solo se Gemini è disponibile
  ...
  def check_unsaved_changes(action_type)
   ...
   # Recupera il messaggio principale interpolando l'azione
   # message = @i18n.t("ui.dialogs.unsaved_message") % { action: action_text }
  ...
  unless @current_file_path
     ...
     @i18n.t("ui.dialogs.unsaved_message") % { action: action_text }
)
    return
  end
  ...
  begin
    ...
    if saved_content != content
      show_save_before_action_dialog(
        action_type,
        @i18n.t('ui.dialogs.modified_title'),
        @i18n.t('ui.dialogs.modified_message') % {filename: File.basename(@current_file_path), action: action_text}
      )
    else
      perform_action(action_type)
    end
    ...
    show_save_before_action_dialog(
      action_type,
      @i18n.t('ui.dialogs.modified_title'),
      @i18n.t('ui.dialogs.modified_message2') % {filename: File.basename(@current_file_path), action: action_text}
    )
  end
end
  ...
  def setup_menu_bar
    ...
    # Menu AI con sottomenu separati
if @ai_configured
  ai_menu = Gio::Menu.new
 ...
 ai_menu.append_submenu("🚀 #{@ai_providers[:groq][:name]}", groq_submenu)
  ...
  menu_model.append_submenu('AI', ai_menu)
end
    settings_submenu = Gio::Menu.new
    settings_submenu.append(@i18n.t('ui.menu.enable_replacement'), 'app.toggle_replacement')
    settings_submenu.append(@i18n.t('ui.menu.enable_completion'), 'app.toggle_word_completion') # <-- RIGA AGGIUNTA
    settings_submenu.append(@i18n.t('ui.menu.enable_sound'), 'app.toggle_sound')

    settings_submenu.append(@i18n.t('ui.menu.enable_grammar'), 'app.toggle_grammar_check')

    settings_submenu.append(@i18n.t('ui.menu.show_replacements'), 'app.show_replacements')
    settings_submenu.append(@i18n.t('ui.menu.cleanup_autosaves'), 'app.cleanup_autosaves')

    menu_model.append_submenu(@i18n.t('ui.menu.settings'), settings_submenu)
  ...
  def setup_menu_actions
    ...
    # Azioni Groq
if @ai_providers[:groq]
  @app.add_action(Gio::SimpleAction.new('groq_sinonimo', nil).tap { |a|
    a.signal_connect('activate') { process_selected_text_with_ai(:groq, 'sinonimo') }
  })
  @app.add_action(Gio::SimpleAction.new('groq_migliora', nil).tap { |a|
    a.signal_connect('activate') { process_selected_text_with_ai(:groq, 'migliora testo') }
  })
  @app.add_action(Gio::SimpleAction.new('groq_riscrivi', nil).tap { |a|
    a.signal_connect('activate') { process_selected_text_with_ai(:groq, 'riscrivi testo') }
  })
  @app.add_action(Gio::SimpleAction.new('groq_arrichisci', nil).tap { |a|
    a.signal_connect('activate') { process_selected_text_with_ai(:groq, 'arrichisci testo') }
  })
end

# Azioni Gemini
if @ai_providers[:gemini]
  @app.add_action(Gio::SimpleAction.new('gemini_sinonimo', nil).tap { |a|
    a.signal_connect('activate') { process_selected_text_with_ai(:gemini, 'sinonimo') }
  })
  @app.add_action(Gio::SimpleAction.new('gemini_migliora', nil).tap { |a|
    a.signal_connect('activate') { process_selected_text_with_ai(:gemini, 'migliora testo') }
  })
  @app.add_action(Gio::SimpleAction.new('gemini_riscrivi', nil).tap { |a|
    a.signal_connect('activate') { process_selected_text_with_ai(:gemini, 'riscrivi testo') }
  })
  @app.add_action(Gio::SimpleAction.new('gemini_arrichisci', nil).tap { |a|
    a.signal_connect('activate') { process_selected_text_with_ai(:gemini, 'arrichisci testo') }
  })
end
 ...
 def process_selected_text_with_ai(provider, ai_action)
  unless @ai_providers[provider]
    show_error_dialog("Provider AI '#{provider}' non configurato.")
    return
  end
  ...
  if bounds.nil? || bounds[0].offset == bounds[1].offset
    show_error_dialog("Nessun testo selezionato.")
    return
  end

  selected_text = buffer.get_text(bounds[0], bounds[1], false)
  provider_name = @ai_providers[provider][:name]

  waiting_dialog = show_waiting_message("Elaborazione con #{provider_name}...\nAttendere prego.")
  ...
  Thread.new do
    begin
      ...
      # Chiama il metodo appropriato in base al provider
      ai_response = case provider
                    when :groq
                      generate_content_with_groq(full_prompt)
                    when :gemini
                      generate_content_with_gemini(full_prompt)
                    else
                      "Errore: provider sconosciuto"
                    end

      GLib::Idle.add do
        waiting_dialog.destroy
        show_scrollable_message("Risultato #{provider_name}", ai_response)
        false
      end
    rescue StandardError => e
      GLib::Idle.add do
...
#    " - [AI: #{@groq_model}]" : ""
  ...
  @root.set_title(
    "#{base_title}#{file_part}#{font_part}#{language_part}"
  )
end
  ...
  unless provider && provider[:api_key]
    return "Errore: La chiave API Groq non è impostata."
  end

  response = HTTPX.post(provider[:url],
    headers: {
    ...
    json: {
      ...
      "messages" => [
        {
          "role" => "system",
          "content" => "Sei un assistente esperto in lingua italiana. SENZA usare formattazione Markdown."
        },
        {
          "role" => "user",
          "content" => prompt
  ...
  when 200
    ...
    return text.gsub('*', '')
  when 400 then "Errore 400: Richiesta non valida.\n\nDettagli: #{response.body}"
  when 401 then "Errore 401: API Key non valida."
  when 403 then "Errore 403: Accesso negato."
  when 404 then "Errore 404: Modello non trovato."
  when 429 then "Errore 429: Troppe richieste."
  when 500 then "Errore 500: Problema server Groq."
  else "Errore #{response.status}: #{response.body}"
  end
rescue StandardError => e
  "Errore Groq: #{e.message}\n#{e.backtrace.first(3).join("\n")}"
end

def generate_content_with_gemini(prompt)
  provider = @ai_providers[:gemini]
  unless provider && provider[:api_key]
    return "Errore: La chiave API Gemini non è impostata."
  end

  url = "#{provider[:url_base]}?key=#{provider[:api_key]}"

  # USA HTTPX (come Groq e PC-1-Text originale)
  ...
  when 200
    data = JSON.parse(response.body)
    text = data.dig("candidates", 0, "content", "parts", 0, "text") || "Risposta vuota da Gemini."
    return text.gsub('*', '').gsub('#', '')
  when 400 then "Errore 400 Gemini: Richiesta non valida.\n\nDettagli: #{response.body}"
  when 401 then "Errore 401 Gemini: API Key non valida."
  when 403 then "Errore 403 Gemini: Accesso negato."
  when 404 then "Errore 404 Gemini: Modello non trovato."
  when 429 then "Errore 429 Gemini: Limite richieste superato."
  when 500 then "Errore 500 Gemini: Problema server Google."
  else "Errore Gemini #{response.status}: #{response.body}"
  end
rescue StandardError => e
  "Errore Gemini: #{e.message}\n#{e.backtrace.first(3).join("\n")}"
end
  ...
  def show_scrollable_message(title, message)
    ...
    scrolled_window.set_child(text_view)
     ...
     # Applica un font più grande alla TextView
    css_provider = Gtk::CssProvider.new
    css_data = "textview { font-family: 'roboto'; font-size: 13pt; }"
    css_provider.load_from_data(css_data)
    text_view.style_context.add_provider(css_provider, Gtk::StyleProvider::PRIORITY_APPLICATION)
